...
- `ControlMaster auto` + `ControlPath` + `ControlPersist 12h` are what let `hpc_login.sh` warm a socket that survives ~12 h. Without them, every command re-prompts for the OTP.
