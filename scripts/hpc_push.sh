...
#
# Usage:
#   bash hpc_push.sh [--dry-run]                      # push HPC_PUSH_PATHS -> $HPC_REMOTE_ROOT/$HPC_CODE_SUBDIR
#   bash hpc_push.sh [--dry-run] <local>... <dest>    # push given local paths -> $HPC_REMOTE_ROOT/<dest>
#
...
rc=0
rsync -azP "${DRY[@]}" "${backup[@]}" "${EXCLUDES[@]}" "${locals[@]}" "$HPC_HOST:$remote/" || rc=$?
